...
//kernel code
__global__ void block_max(int *a, int no,int N, int *b, int *gapX, int *gapY)

{
    __shared__ int mat[3][3][3];
    int index_i=0,index_j=0;

    //Calculating global indices for accesing elements in score matrix, a
    if(no<N/2)
    {
        index_i=2*blockIdx.x;
        index_j=2*no-index_i;
    }
    else
    {
        index_i=(blockIdx.x+no-N/2+1)*2;
        index_j=2*no-index_i;
    }
    ...
    if(threadIdx.x==0)  //one block along first anti-diagonal
    {

      if(threadIdx.y==0)
       	{
	       mat[1][1][0]+=maximum(mat[0][1][0],mat[0][1][1]+gapY[index_j+1],mat[0][1][2]+gapY[index_j+1],&b[index+(N+1)*3+1*3+threadIdx.y]);
	   }

        if(threadIdx.y==1)
         {
            mat[1][1][1]+=maximum(mat[1][0][0]+gapX[index_i+1],mat[1][0][1],mat[1][0][2]+gapX[index_i+1],&b[index+(N+1)*3+1*3+threadIdx.y]);	
        }

        if(threadIdx.y==2)
        {
            mat[1][1][2]+=maximum(mat[0][0][0],mat[0][0][1],mat[0][0][2],&b[index+(N+1)*3+1*3+threadIdx.y]);
        }
    }

    //two blocks along second anti-diagonal
    if(threadIdx.x<=1)
    {
        if(threadIdx.y==0)
            mat[threadIdx.x+1][2-threadIdx.x][threadIdx.y]+=maximum(mat[threadIdx.x][2-threadIdx.x][0],mat[threadIdx.x][2-threadIdx.x][1]+gapY[index_j+2-threadIdx.x],mat[threadIdx.x][2-threadIdx.x][2]+gapY[index_j+2-threadIdx.x],&b[index+3*(N+1)*(2-threadIdx.x)+3*(threadIdx.x+1)+threadIdx.y]);

        if(threadIdx.y==1)
            mat[threadIdx.x+1][2-threadIdx.x][threadIdx.y]+=maximum(mat[threadIdx.x+1][1-threadIdx.x][0]+gapX[index_i+1+threadIdx.x],mat[threadIdx.x+1][1-threadIdx.x][1],mat[threadIdx.x+1][1-threadIdx.x][2]+gapX[index_i+1+threadIdx.x],&b[index+3*(N+1)*(2-threadIdx.x)+3*(threadIdx.x+1)+threadIdx.y]);

        if(threadIdx.y==2)
            mat[threadIdx.x+1][3-threadIdx.x-1][threadIdx.y]+=maximum(mat[threadIdx.x][1-threadIdx.x][0],mat[threadIdx.x][1-threadIdx.x][1],mat[threadIdx.x][1-threadIdx.x][2],&b[index+3*(N+1)*(2-threadIdx.x)+3*(threadIdx.x+1)+threadIdx.y]);
 }

    //one block in the third anti-diagonal
    if(threadIdx.x==2)
    {
        if(threadIdx.y==0){
          mat[2][2][threadIdx.y]+=maximum(mat[1][2][0],mat[1][2][1]+gapY[index_j+2],mat[1][2][2]+gapY[index_j+2],&b[index+2*(N+1)*3+2*3+threadIdx.y]);
        }

        if(threadIdx.y==1){
            mat[2][2][threadIdx.y]+=maximum(mat[2][1][0]+gapX[index_i+2],mat[2][1][1],mat[2][1][2]+gapX[index_i+2],&b[index+2*(N+1)*3+2*3+threadIdx.y]);

        }
        if(threadIdx.y==2){
            mat[2][2][threadIdx.y]+=maximum(mat[1][1][0],mat[1][1][1],mat[1][1][2],&b[index+2*(N+1)*3+2*3+threadIdx.y]);
         }
}
__syncthreads();
...
//Creating variables for device memory
 int *dev_a, *dev_b, *dev_gapX, *dev_gapY;
    ...
    dim3 thread_count(3,3);

    struct stopwatch_t* timer=NULL;
    long double t_gpu;
    stopwatch_init();

    //Timing GPU; kernel calls
    ...
    for(int i=0;i<N-1;i++)
    {
        if(i<N/2)
        {   
            block_max<<<i+1,thread_count>>>(dev_a,i,N,dev_b,dev_gapX,dev_gapY);
        }
       else
        {
            block_max<<<N-i-1-N%2,thread_count>>>(dev_a,i,N,dev_b,dev_gapX,dev_gapY);
        }
    }
...
//Handling extra computation when size not a multiple of two
if(N%2!=0)
{
