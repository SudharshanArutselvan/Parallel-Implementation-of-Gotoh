...
gth_Arr gth_init(size_t lenX, size_t lenY) {
    gth_Arr arr;
//cout<<"INIT ARRAY inside function: "<<endl;
    arr.lenX = lenX;
    arr.lenY = lenY;
...
//cout<<"Inside subs function: "<<endl;
    for (size_t i=1 ; i<=arr.lenX ; i++) {
        for (size_t j=1 ; j<=arr.lenY ; j++) {
            arr.data[i][j][2] = score[resX[i-1] - 'A'][resY[j-1] - 'A'];
//		cout<<"DATA: "<<i<<" "<<j<<" "<<arr.data[i][j][2]<<endl;
        }
    }
}
...
//cout<<"HERE??";
 ...
 //   cout<<"Filling OPT1: 0 to 0 ; 1 to arr.lenY-1"<<endl;
...
/*int gth_align(gth_Arr arr) {
    ...
    //recur(arr.data,arr.path,arr.lenX,arr.lenY);
...
void gth_free(gth_Arr arr) {
    free_arr(arr.data, arr.lenX+1, arr.lenY+1);
    free_arr(arr.path, arr.lenX+1, arr.lenY+1);
    free(arr.gapX);
    free(arr.gapY);
}
